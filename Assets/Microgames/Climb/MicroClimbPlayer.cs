...
public class MicroClimbPlayer : MonoBehaviour
{
    ...
    {
        if (collision.tag != "Goal" || microClimbManager.cleared || microClimbManager.timer <= 0) return;
        microClimbManager.cleared = true;
        microClimbManager.End();
        microClimbManager.sfx.PlaySFX(0);
    }
}
